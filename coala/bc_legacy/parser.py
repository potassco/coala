		...
		if len(head) > 0:
			if nfalse(head):
				if nfalse(ifpart) and nfalse(ifcons):
					self.data['static_laws'].append(self.add_meta((head,ifpart,ifcons,wherepart,wheriab,variab),t.lineno(1)))
			else:
				if nfalse(ifpart) and nfalse(ifcons):
					self.data['impossible_laws'].append(self.add_meta((ifpart,ifcons,wherepart,wheriab,variab),t.lineno(1)))
...
#					 | formula CAUSES formula IF formula where_part'''
				...
				if len(t[3]) > 0:
					if nfalse(t[3]):
						if nfalse(t[1]):
							...
							t[3], var3 = unpack(t[3])
							t[4], var4 = unpack(t[4])
							self.data['dynamic_laws'].append(self.add_meta((t[3],t[1],None,t[4],var4,var3+var1),t.lineno(1)))
				else:
					t[1], var1 = unpack(t[1])
					t[4], var4 = unpack(t[4])
					self.data['nonexecutable_laws'].append(self.add_meta((t[1],None,None,t[4],var4,var1),t.lineno(1)))
			else: 
				if len(t[3]) > 0:
					if nfalse(t[3]):
						if nfalse(t[1]) and nfalse(t[5]):
							...
							t[5], var5 = unpack(t[5])
							t[6], var6 = unpack(t[6])
							self.data['dynamic_laws'].append(self.add_meta((t[3],t[1]+t[5],None,t[6],var6,var1+var3+var5),t.lineno(1)))
				else:
					t[1], var1 = unpack(t[1])
					t[5], var5 = unpack(t[5])
					t[6], var6 = unpack(t[6])
					self.data['nonexecutable_laws'].append(self.add_meta((t[1]+t[5],None,None,t[6],var6,var1+var5),t.lineno(1)))
		else:
			if len(t[1]) > 0:
				if nfalse(t[1]):
					if nfalse(t[2]) and nfalse(t[3]):
						...
						t[3], var3 = unpack(t[3])
						t[4], var4 = unpack(t[4])
						self.data['dynamic_laws'].append(self.add_meta((t[1],t[2],t[3],t[4],var4,var1+var2+var3),t.lineno(1)))
				else:
					t[2], var2 = unpack(t[2])
					t[3], var3 = unpack(t[3])
					t[4], var4 = unpack(t[4])
					self.data['nonexecutable_laws'].append(self.add_meta((t[2],None,t[3],t[4],var4,var2+var3),t.lineno(1)))
	...
	def p_inertial_law(self,t):
		...
		t[2], var2 = unpack(t[2])
		t[3], var3 = unpack(t[3])
		self.data['inertial_laws'].append(self.add_meta((t[2],t[3],var3,var2),t.lineno(1)))
		...
		if len(t[2]) > 0:
			if nfalse(t[2]):
				if len(t) == 4: 
					t[2], var2 = unpack(t[2])
					t[3], var3 = unpack(t[3])
					self.data['default_laws'].append(self.add_meta((t[2],None,None,t[3],var3,var2),t.lineno(1)))
				elif t[3] == '<if>': 
					if nfalse(t[4]):
						t[2], var2 = unpack(t[2])
						t[4], var4 = unpack(t[4])
						t[5], var5 = unpack(t[5])
						self.data['default_laws'].append(self.add_meta((t[2],t[4],None,t[5],var5,var2+var4),t.lineno(1)))
				else: 
					if nfalse(t[4]):
						t[2], var2 = unpack(t[2])
						t[4], var4 = unpack(t[4])
						t[5], var5 = unpack(t[5])
						self.data['default_laws'].append(self.add_meta((t[2],None,t[4],t[5],var5,var2+var4),t.lineno(1)))
		...
		if len(t[2]) > 0:
			if nfalse(t[2]):
				if nfalse(t[3]):
					...
					t[3], var3 = unpack(t[3])
					t[4], var4 = unpack(t[4])
					self.data['impossible_laws'].append(self.add_meta((t[2],t[3],t[4],var4,var2+var3),t.lineno(1)))
		else:
		...
		if len(t[2]) > 0:
			if nfalse(t[2]):
				if nfalse(t[3]) and nfalse(t[4]):
					...
					t[4], var4 = unpack(t[4])
					t[5], var5 = unpack(t[5])
					self.data['nonexecutable_laws'].append(self.add_meta((t[2],t[3],t[4],t[5],var5,var2+var3+var4),t.lineno(1)))
		else:
...
##########################
	...
	def p_binding(self,t): #TODO: Maybe add ESCAPE_BRACELETS
		''' binding : ACT fluent
...
#################
	...
	def build(self,**kwargs):
		...
		if self.debug:
			...
			self.parser = yacc.yacc(module=self, debug=False, outputdir=mypath, optimize=False, write_tables=False, **kwargs) # SILENT MODE!
